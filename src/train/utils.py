...
def setup_logging(
    log_path: Union[str, Path] = './training.log', log_level: str = 'INFO'
) -> None:
...
def plot_training_history(
    history: list[dict[str, Any]], output_dir: Union[str, Path], exp_name: str
) -> tuple[Figure, Figure]:
    ...
    if not avg_df.empty:
        ...
        if (
            'avg_val_loss' in avg_df.columns
            and not avg_df['avg_val_loss'].isnull().all()
        ):
            ax_avg1.plot(
                avg_df['step'],
                avg_df['avg_val_loss'],
